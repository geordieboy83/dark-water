...
void GLPolygon::objload(fstream& file)
{
	...
	while (getline(file,s)){
		...
		while(s.c_str()[0]=='f'&s.c_str()[1]==' '){
			...
			//if(test[0]!=-1&&test[1]!=-1&&test[2]!=-1) cout << "Vertex/texture-coordinate/normal" << endl;
			//else if(test[3]!=-1&&test[4]!=-1) cout << "Vertex/normal" << endl;
			//else if(test[5]!=-1&&test[6]!=-1) cout << "Vertex/texture-coordinate" << endl;
