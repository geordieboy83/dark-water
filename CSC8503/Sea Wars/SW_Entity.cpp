...
void Entity::quad(Vertex position, Vertex origin, int* quad_idxs)
{
	...
	quad_idxs[0]=(int)floor((float)(minx-SW_WORLD_FROM)/quadstep);
	quad_idxs[1]=(int)ceil((float)(maxx-SW_WORLD_FROM)/quadstep);
	quad_idxs[2]=(int)floor((float)(miny-SW_WORLD_FROM)/quadstep);
	quad_idxs[3]=(int)ceil((float)(maxy-SW_WORLD_FROM)/quadstep);	
	return;
}
...
void Entity::wound(int damage)
{
	...
	if(HP<=0)die();
		...
		chaindesign((float)(max(default_hps-HP,0))*SW_DEATHFACTOR/(float) default_hps,stuff);		
	}
}
